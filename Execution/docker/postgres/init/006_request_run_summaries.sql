create table if not exists request_run_summaries (
    ...
    constraint request_run_summaries_request_id_nonempty_check
        check (length(btrim(request_id)) > 0),
    constraint request_run_summaries_run_id_nonempty_check
        check (length(btrim(run_id)) > 0),
    constraint request_run_summaries_trace_id_nonempty_check
        check (length(btrim(trace_id)) > 0),
    constraint request_run_summaries_raw_query_nonempty_check
        check (length(btrim(raw_query)) > 0),
    constraint request_run_summaries_normalized_query_nonempty_check
        check (length(btrim(normalized_query)) > 0),
    constraint request_run_summaries_pipeline_config_version_nonempty_check
        check (length(btrim(pipeline_config_version)) > 0),
    constraint request_run_summaries_corpus_version_nonempty_check
        check (length(btrim(corpus_version)) > 0),
    constraint request_run_summaries_retriever_version_nonempty_check
        check (length(btrim(retriever_version)) > 0),
    constraint request_run_summaries_retriever_kind_check
        check (retriever_kind in ('Dense', 'Hybrid')),
    constraint request_run_summaries_embedding_model_nonempty_check
        check (length(btrim(embedding_model)) > 0),
    constraint request_run_summaries_prompt_template_id_nonempty_check
        check (length(btrim(prompt_template_id)) > 0),
    constraint request_run_summaries_prompt_template_version_nonempty_check
        check (length(btrim(prompt_template_version)) > 0),
    constraint request_run_summaries_generation_model_nonempty_check
        check (length(btrim(generation_model)) > 0),
    constraint request_run_summaries_final_answer_nonempty_check
        check (length(btrim(final_answer)) > 0),
    constraint request_run_summaries_answer_completeness_label_nonempty_check
        check (answer_completeness_label is null or length(btrim(answer_completeness_label)) > 0),
    constraint request_run_summaries_groundedness_label_nonempty_check
        check (groundedness_label is null or length(btrim(groundedness_label)) > 0),
    constraint request_run_summaries_answer_relevance_label_nonempty_check
        check (answer_relevance_label is null or length(btrim(answer_relevance_label)) > 0),
    constraint request_run_summaries_correct_refusal_label_nonempty_check
        check (correct_refusal_label is null or length(btrim(correct_refusal_label)) > 0),
    constraint request_run_summaries_judge_generation_model_nonempty_check
        check (judge_generation_model is null or length(btrim(judge_generation_model)) > 0),
    constraint request_run_summaries_judge_generation_prompt_version_nonempty_check
        check (
            judge_generation_prompt_version is null
            or length(btrim(judge_generation_prompt_version)) > 0
        ),
    constraint request_run_summaries_judge_retrieval_model_nonempty_check
        check (judge_retrieval_model is null or length(btrim(judge_retrieval_model)) > 0),
    constraint request_run_summaries_judge_retrieval_prompt_version_nonempty_check
        check (
            judge_retrieval_prompt_version is null
            or length(btrim(judge_retrieval_prompt_version)) > 0
        ),
    constraint request_run_summaries_input_token_count_check
        check (input_token_count >= 1),
    constraint request_run_summaries_top_k_requested_check
        check (top_k_requested >= 1),
    constraint request_run_summaries_prompt_tokens_check
        check (prompt_tokens >= 0),
    constraint request_run_summaries_completion_tokens_check
        check (completion_tokens >= 0),
    constraint request_run_summaries_total_tokens_nonnegative_check
        check (total_tokens >= 0),
    constraint request_run_summaries_total_tokens_sum_check
        check (total_tokens = prompt_tokens + completion_tokens),
    constraint request_run_summaries_retrieval_relevant_count_check
        check (retrieval_relevance_relevant_count is null or retrieval_relevance_relevant_count >= 0),
    constraint request_run_summaries_retrieval_selected_count_check
        check (retrieval_relevance_selected_count is null or retrieval_relevance_selected_count >= 0),
    constraint request_run_summaries_retrieval_chunk_count_check
        check (retrieval_chunk_count is null or retrieval_chunk_count >= 0),
    constraint request_run_summaries_retrieval_selected_le_chunk_count_check
        check (
    ...
    constraint request_run_summaries_retrieval_relevant_le_chunk_count_check
        check (
...
create index if not exists idx_request_run_summaries_run_id
    on request_run_summaries (run_id);

create index if not exists idx_request_run_summaries_source_received_at
    on request_run_summaries (source_received_at);

create index if not exists idx_request_run_summaries_trace_id
    on request_run_summaries (trace_id);

create index if not exists idx_request_run_summaries_pipeline_config_version
    on request_run_summaries (pipeline_config_version);

create index if not exists idx_request_run_summaries_retriever_version
    on request_run_summaries (retriever_version);

create index if not exists idx_request_run_summaries_prompt_template_version
    on request_run_summaries (prompt_template_version);

create index if not exists idx_request_run_summaries_generation_model
    on request_run_summaries (generation_model);

create index if not exists idx_request_run_summaries_run_received
    on request_run_summaries (run_id, source_received_at);

create index if not exists idx_request_run_summaries_run_generation_model
    on request_run_summaries (run_id, generation_model);

create index if not exists idx_request_run_summaries_run_retriever_version
    on request_run_summaries (run_id, retriever_version);

alter table request_run_summaries
    add column if not exists retriever_kind text;

update request_run_summaries
...
alter table request_run_summaries
    drop constraint if exists request_run_summaries_retriever_kind_check;

alter table request_run_summaries
    add constraint request_run_summaries_retriever_kind_check
        check (retriever_kind in ('Dense', 'Hybrid'));

create index if not exists idx_request_run_summaries_retriever_kind
    on request_run_summaries (retriever_kind);

create index if not exists idx_request_run_summaries_run_retriever_kind
    on request_run_summaries (run_id, retriever_kind);
