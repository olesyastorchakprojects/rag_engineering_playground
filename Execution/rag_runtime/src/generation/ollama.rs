...
impl GenerationTransport for OllamaTransport {
    async fn complete(
        ...
        let settings = match transport {
            TransportSettings::Ollama(settings) => settings,
            _ => {
                return Err(GenerationError::UnexpectedInternalState {
                    message: "ollama transport received non-ollama transport settings".to_string(),
                });
            }
        };

        let url = format!("{}/api/chat", settings.url.trim_end_matches('/'));
