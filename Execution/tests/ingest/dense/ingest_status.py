...
def build_chunk(chunk_index: int, section_title: str, content_hash: str, text: str) -> dict:
    return {
        "schema_version": 1,
        ...
        "section_path": ["Introduction", section_title],
...
def build_chunks():
    update_chunk = build_chunk(0, "Original Update Section", "sha256:update-original", "Update text")
    skip_and_log_chunk = build_chunk(1, "Skip And Log Section", "sha256:skip-and-log-original", "Skip and log text")
    skip_chunk = build_chunk(2, "Skip Section", "sha256:skip-original", "Skip text")
    insert_chunk = build_chunk(3, "Insert Section", "sha256:insert-original", "Insert text")
    return update_chunk, skip_and_log_chunk, skip_chunk, insert_chunk
...
def ingest_object_matches(config: dict, first_payload: dict, second_payload: dict) -> bool:
    first_ingest = first_payload.get("ingest")
    second_ingest = second_payload.get("ingest")
    if not isinstance(first_ingest, dict) or not isinstance(second_ingest, dict):
        return False
    if second_ingest.get("embedding_model") != config["embedding"]["model"]["name"]:
        return False
    if second_ingest.get("embedding_model_dimension") != config["embedding"]["model"]["dimension"]:
        return False
    if second_ingest.get("ingest_config_version") != config["pipeline"]["ingest_config_version"]:
        return False
    if not is_valid_iso_utc_timestamp(second_ingest.get("ingested_at")):
        return False
    if second_ingest.get("ingested_at") == first_ingest.get("ingested_at"):
        return False
    return True
...
def run_case(script_path: Path, chunk_schema_path: Path, config_path: Path, env_file_path: Path):
    ...
    collection_name = f"{config['qdrant']['collection']['name']}_status_e2e_{uuid.uuid4().hex[:10]}"
    cleanup_error = None
    first_run_stdout = ""
    first_run_stderr = ""
    second_run_stdout = ""
    ...
    actual_skipped_log_entries = None
    ...
    update_chunk, skip_and_log_chunk, skip_chunk, insert_chunk = build_chunks()
    all_chunks = [update_chunk, skip_and_log_chunk, skip_chunk, insert_chunk]
    point_ids = {chunk["chunk_id"]: compute_point_id(config, chunk) for chunk in all_chunks}
    update_point_id = point_ids[update_chunk["chunk_id"]]
    skip_and_log_point_id = point_ids[skip_and_log_chunk["chunk_id"]]
    skip_point_id = point_ids[skip_chunk["chunk_id"]]
    insert_point_id = point_ids[insert_chunk["chunk_id"]]

    with tempfile.TemporaryDirectory() as tmp_dir_str:
        tmp_dir = Path(tmp_dir_str)
        first_chunks_path = tmp_dir / "first_chunks.jsonl"
        second_chunks_path = tmp_dir / "second_chunks.jsonl"
        ...
        write_chunks_jsonl(first_chunks_path, [update_chunk, skip_and_log_chunk, skip_chunk])
        write_chunks_jsonl(second_chunks_path, [update_chunk, skip_and_log_chunk, skip_chunk, insert_chunk])

        first_process = run_ingest(script_path, first_chunks_path, mutated_config_path, env_file_path)
        ...
        if first_process.returncode != 0:
            return False, {
                ...
                "actual_skipped_log_entries": actual_skipped_log_entries,
                "cleanup_error": cleanup_error,
            }

        collection_status_code, _, collection_json = get_collection(qdrant_url, collection_name)
        if collection_status_code != 200 or extract_points_count(collection_json) != 3:
            return False, {
                "error": "first run did not create exactly 3 points",
                "collection_name": collection_name,
                "first_run_stdout": first_run_stdout,
                "first_run_stderr": first_run_stderr,
                "second_run_stdout": second_run_stdout,
        ...
        snapshots = {}
        for point_id in (update_point_id, skip_and_log_point_id, skip_point_id):
            ...
            if point_status_code != 200 or point_status != "ok" or actual_point_id != point_id or not isinstance(payload, dict):
                return False, {
                    "error": f"failed to read point snapshot point_id={point_id}",
                    "collection_name": collection_name,
                    "first_run_stdout": first_run_stdout,
                    "first_run_stderr": first_run_stderr,
                    "second_run_stdout": second_run_stdout,
            ...
            snapshots[point_id] = json.loads(json.dumps(payload))

        update_payload_snapshot = json.loads(json.dumps(snapshots[update_point_id]))
        update_payload_snapshot["section_title"] = "Mutated Section Title"
        update_status, update_body, _ = update_point_payload(
            qdrant_url,
            collection_name,
            update_point_id,
            update_payload_snapshot,
        )
        if update_status < 200 or update_status >= 300:
            return False, {
                "error": f"failed to mutate update point body={update_body}",
                "collection_name": collection_name,
                "first_run_stdout": first_run_stdout,
                "first_run_stderr": first_run_stderr,
                "second_run_stdout": second_run_stdout,
        ...
        skip_and_log_payload_snapshot = json.loads(json.dumps(snapshots[skip_and_log_point_id]))
        skip_and_log_payload_snapshot["content_hash"] = "sha256:mutated-content-hash"
        skip_and_log_status, skip_and_log_body, _ = update_point_payload(
            qdrant_url,
            collection_name,
            skip_and_log_point_id,
            skip_and_log_payload_snapshot,
        )
        if skip_and_log_status < 200 or skip_and_log_status >= 300:
            return False, {
                "error": f"failed to mutate skip_and_log point body={skip_and_log_body}",
                "collection_name": collection_name,
                "first_run_stdout": first_run_stdout,
                "first_run_stderr": first_run_stderr,
                "second_run_stdout": second_run_stdout,
        ...
        second_process = run_ingest(script_path, second_chunks_path, mutated_config_path, env_file_path)
        ...
        result_points = {}
        for point_id in (update_point_id, skip_and_log_point_id, skip_point_id, insert_point_id):
            point_status_code, _, point_json = get_point(qdrant_url, collection_name, point_id)
            point_status, actual_point_id, payload = extract_point_payload(point_json)
            result_points[point_id] = {
                "http_status": point_status_code,
                "status": point_status,
                "id": actual_point_id,
                "payload": payload,
            }
        ...
        actual_skipped_log_entries = len(skipped_entries)
        failed_entries = read_jsonl_lines(failed_log_path)

    if second_process.returncode != 0:
        return False, {
            "error": "second ingest subprocess failed",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if final_collection_status != 200 or actual_points_count != 4:
        return False, {
    ...
    for required_substring in ("created=1", "updated=1", "unchanged=1", "skipped=1", "failed=0"):
    ...
    insert_result = result_points[insert_point_id]
    if insert_result["http_status"] != 200 or insert_result["status"] != "ok" or insert_result["id"] != insert_point_id:
        return False, {
            "error": "insert point missing after second run",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    update_payload = result_points[update_point_id]["payload"]
    if not isinstance(update_payload, dict):
        return False, {
            "error": "update point payload missing",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if update_payload.get("section_title") != update_chunk["section_title"]:
        return False, {
            "error": "update point section_title mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if update_payload.get("content_hash") != update_chunk["content_hash"]:
        return False, {
            "error": "update point content_hash mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if update_payload.get("text") != update_chunk["text"]:
        return False, {
            "error": "update point text mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if not ingest_object_matches(mutated_config, snapshots[update_point_id], update_payload):
        return False, {
            "error": "update point ingest object mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    skip_payload = result_points[skip_point_id]["payload"]
    if skip_payload != snapshots[skip_point_id]:
        return False, {
            "error": "skip point payload changed",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    skip_and_log_payload = result_points[skip_and_log_point_id]["payload"]
    if not isinstance(skip_and_log_payload, dict):
        return False, {
            "error": "skip_and_log point payload missing",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if skip_and_log_payload.get("content_hash") != "sha256:mutated-content-hash":
        return False, {
            "error": "skip_and_log point content_hash mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if skip_and_log_payload.get("content_hash") == skip_and_log_chunk["content_hash"]:
        return False, {
            "error": "skip_and_log point was overwritten by incoming chunk",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if actual_skipped_log_entries != 1:
        return False, {
            "error": "unexpected skipped log entry count",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    skipped_entry = skipped_entries[0]
    if skipped_entry.get("reason") != "fingerprint_changed":
        return False, {
            "error": "skipped log reason mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
    ...
    if skipped_entry.get("chunk_id") != skip_and_log_chunk["chunk_id"]:
        return False, {
            "error": "skipped log chunk_id mismatch",
            "collection_name": collection_name,
            "first_run_stdout": first_run_stdout,
            "first_run_stderr": first_run_stderr,
            "second_run_stdout": second_run_stdout,
...
def main() -> None:
    ...
    if ok:
        print("OK [ingest_status_matrix]")
        print("cases=1 failed=0 passed=1")
        raise SystemExit(0)

    print("FAIL [ingest_status_matrix]")
    ...
    print(f"actual_skipped_log_entries={details['actual_skipped_log_entries']}")
