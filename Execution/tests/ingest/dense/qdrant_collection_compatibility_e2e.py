...
def get_collection(qdrant_url: str, collection_name: str):
    ...
    return send_json_request("GET", qdrant_url.rstrip("/") + f"/collections/{quoted}")


def delete_collection(qdrant_url: str, collection_name: str):
    quoted = urllib.parse.quote(collection_name, safe="")
    return send_json_request("DELETE", qdrant_url.rstrip("/") + f"/collections/{quoted}")


def create_collection(qdrant_url: str, collection_name: str, size: int, distance: str, metadata):
    quoted = urllib.parse.quote(collection_name, safe="")
    body = {"vectors": {"size": size, "distance": distance}}
    if metadata is not None:
        body["metadata"] = metadata
    return send_json_request("PUT", qdrant_url.rstrip("/") + f"/collections/{quoted}", body)
...
def run_one_case(script_path: Path, config_path: Path, env_file_path: Path, case_name: str):
    config = load_toml(config_path)
    env = parse_env_file(env_file_path)
    qdrant_url = env["QDRANT_URL"]
    embedding_url = env["OLLAMA_URL"]
    collection_name = f"{config['qdrant']['collection']['name']}_{case_name}_{uuid.uuid4().hex[:8]}"
    expected_model = config["embedding"]["model"]["name"]
    expected_dim = config["embedding"]["model"]["dimension"]
    expected_distance = config["qdrant"]["collection"]["distance"]

    metadata = {"embedding_model_name": expected_model}
    vector_dim = expected_dim
    vector_distance = expected_distance
    expected_error_substring = ""
    if case_name == "collection_metadata_missing":
        metadata = None
        expected_error_substring = "collection metadata missing"
    elif case_name == "embedding_model_name_mismatch":
        metadata = {"embedding_model_name": "different-model-for-test"}
        expected_error_substring = "collection embedding_model_name mismatch"
    elif case_name == "vector_dimension_mismatch":
        vector_dim = expected_dim + 1
        expected_error_substring = "collection vector dimension mismatch"
    elif case_name == "vector_distance_mismatch":
        vector_distance = "Dot" if expected_distance != "Dot" else "Euclid"
        expected_error_substring = "collection vector distance mismatch"
    else:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_str:
        ...
        if status_code != 404:
            return False, {
                "error": f"temporary collection already exists http_status={status_code}",
                "case_name": case_name,
        ...
        create_status, create_body, _ = create_collection(
            qdrant_url,
            collection_name,
            vector_dim,
            vector_distance,
        ...
        if create_status < 200 or create_status >= 300:
            return False, {
                "error": f"failed to create collection http_status={create_status} body={create_body}",
                "case_name": case_name,
                "collection_name": collection_name,
                "stdout": "",
                "stderr": "",
        ...
        get_status, _, get_json = get_collection(qdrant_url, collection_name)
        points_count = extract_points_count(get_json)
        ...
        if process.returncode != 1:
            return False, {
                "error": f"unexpected exit code {process.returncode}",
                "case_name": case_name,
                "collection_name": collection_name,
        ...
        if expected_error_substring not in combined_output:
            return False, {
                "error": "expected error substring not found",
                "case_name": case_name,
                "collection_name": collection_name,
                "stdout": process.stdout.strip(),
                "stderr": process.stderr.strip(),
        ...
        if get_status != 200:
            return False, {
                "error": f"failed to read collection after subprocess http_status={get_status}",
                "case_name": case_name,
                "collection_name": collection_name,
                "stdout": process.stdout.strip(),
                "stderr": process.stderr.strip(),
        ...
        if points_count != 0:
            return False, {
                "error": f"expected points_count=0 actual={points_count}",
                "case_name": case_name,
                "collection_name": collection_name,
                "stdout": process.stdout.strip(),
                "stderr": process.stderr.strip(),
...
def main() -> None:
    ...
    base_config = load_toml(args.config_path)
    env = parse_env_file(args.env_file_path)
    check_qdrant_available(env["QDRANT_URL"])
    check_embedding_available(env["OLLAMA_URL"], base_config["embedding"]["model"]["name"])
    ...
    for case_name in cases:
        ok, details = run_one_case(args.script_path, args.config_path, args.env_file_path, case_name)
        if ok:
            passed += 1
            ...
            print(f"expected_error_substring={details['expected_error_substring']}")
