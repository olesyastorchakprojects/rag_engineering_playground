    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_PUT(self):
            ...
            if self.path == f"/collections/{collection_name}/points":
                points = []
                if isinstance(json_body, dict) and isinstance(json_body.get("points"), list):
                    points = json_body["points"]
                if len(points) == 2:
                    self.send_response(500)
                    self.send_header("Content-Type", "application/json")
                    self.end_headers()
                    self.wfile.write(b'{"status":"error","error":"stub upsert batch failure"}')
                    return
                if len(points) == 1:
                    self.send_response(200)
                    self.send_header("Content-Type", "application/json")
...
def run_case(script_path: Path, chunk_schema_path: Path, config_path: Path, env_file_path: Path):
    ...
    expected_points_lengths = ([initial_batch_item_count] * retry_attempts) + [1, 1]
    case_name = "upsert_batch_fallback"
        ...
        try:
            ...
            while time.time() < deadline:
                with qdrant_state.lock:
                    observed_lengths = [
                        len(request.json_body.get("points", []))
                        for request in qdrant_state.requests
                        if request.method == "PUT"
                        and request.path == f"/collections/{collection_name}/points"
                        and isinstance(request.json_body, dict)
                    ]
                if observed_lengths[: len(expected_points_lengths)] == expected_points_lengths:
                    if len(observed_lengths) >= len(expected_points_lengths):
                        break
                if process.poll() is not None:
                    ...
                    return (
                        ...
                        case_name,
                        {
                            "error": "subprocess exited before reaching expected points lengths sequence",
                            "observed_request_count": len(observed_lengths),
                            "observed_points_lengths": observed_lengths,
                            "expected_points_lengths": expected_points_lengths,
                            "stdout": stdout.strip(),
                            "stderr": stderr.strip(),
                        },
                    )
                ...
                return (
                    ...
                    case_name,
                    {
                        "error": "expected points lengths sequence was not reached before timeout",
                        "observed_request_count": len(observed_lengths),
                        "observed_points_lengths": observed_lengths,
                        "expected_points_lengths": expected_points_lengths,
                    },
            ...
            if observed_lengths[: len(expected_points_lengths)] != expected_points_lengths:
                return (
                    ...
                    case_name,
                    {
                        "error": "observed points lengths sequence does not match expected sequence",
                        "observed_request_count": len(observed_lengths),
                        "observed_points_lengths": observed_lengths,
                        "expected_points_lengths": expected_points_lengths,
                    },
...
def main() -> None:
    ...
    if passed:
        ...
        print(f"observed_points_lengths={details['observed_points_lengths']}")
        if args.verbose:
            print(f"expected_points_lengths={details['expected_points_lengths']}")
